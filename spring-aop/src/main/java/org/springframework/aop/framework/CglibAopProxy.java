...
class CglibAopProxy implements AopProxy, Serializable {
	...
	public Object getProxy(@Nullable ClassLoader classLoader) {
		...
		try {
			...
			//创建及配置Enhancer Configure CGLIB Enhancer...
			...
			//生成代理类以及创建代理 Generate the proxy class and create a proxy instance.
	...
	private Callback[] getCallbacks(Class<?> rootClass) throws Exception {
		//对于expose-proxy属性的代理 Parameters used for optimization choices...
		boolean exposeProxy = this.advised.isExposeProxy();
		boolean isFrozen = this.advised.isFrozen();
		boolean isStatic = this.advised.getTargetSource().isStatic();

		//将拦截器封装在DynamicAdvisedInterceptor中 Choose an "aop" interceptor (used for AOP calls).
		...
		Callback[] mainCallbacks = new Callback[] {
				aopInterceptor,  //将拦截器懒加入Callback中 for normal advice
		...
		public Object intercept(Object proxy, Method method, Object[] args, MethodProxy methodProxy) throws Throwable {
			...
			try {
				...
				if (chain.isEmpty() && Modifier.isPublic(method.getModifiers())) {
					...
					//进入链 We need to create a method invocation...
