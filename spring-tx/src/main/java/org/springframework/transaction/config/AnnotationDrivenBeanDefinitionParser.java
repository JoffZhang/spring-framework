...
class AnnotationDrivenBeanDefinitionParser implements BeanDefinitionParser {
	...
	public BeanDefinition parse(Element element, ParserContext parserContext) {
		...
		//对mode属性的判断，如果我们需要使用AspectJ方式进行事务切入（Spring中的事务是以AOP为基础的），那么可以这样配置
		//<tx:annotation-driven transation-manager="transactionManager" mode="aspectj">
	...
	private static class AopAutoProxyConfigurer {
		...
		public static void configureAutoProxyCreator(Element element, ParserContext parserContext) {
			...
			if (!parserContext.getRegistry().containsBeanDefinition(txAdvisorBeanName)) {
				...
				//创建TranstationAttributeSource的bean Create the TransactionAttributeSource definition.
				RootBeanDefinition sourceDef = new RootBeanDefinition(
						"org.springframework.transaction.annotation.AnnotationTransactionAttributeSource");
				sourceDef.setSource(eleSource);
				sourceDef.setRole(BeanDefinition.ROLE_INFRASTRUCTURE);
				//注册bean，并用spring中的定义规则生成beanName
				String sourceName = parserContext.getReaderContext().registerWithGeneratedName(sourceDef);

				//创建TranstationInterceptor的bean Create the TransactionInterceptor definition.
				...
				//创建TransationAttributeSourceAdvisor的bean Create the TransactionAttributeSourceAdvisor definition.
