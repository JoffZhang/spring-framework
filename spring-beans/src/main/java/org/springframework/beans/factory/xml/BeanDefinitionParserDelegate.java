...
public class BeanDefinitionParserDelegate {
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		try {
			//创建用于正在属性的AbstractBeanDefinition类型的GenericBeanDefinition
	...
	public void parseLookupOverrideSubElements(Element beanEle, MethodOverrides overrides) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			//仅当在spring默认bean子元素下且为<lookup-method 时有效
			if (isCandidateElement(node) && nodeNameEquals(node, LOOKUP_METHOD_ELEMENT)) {
				Element ele = (Element) node;
				//获取需要修饰的方法 及配置返回的bean	封装成LookupOverride
	...
	public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
		...
		//解析constructor-arg上的 ref  value属性
		...
		else {//既没有ref又没有value也没有子元素，spring晕菜了!
	 ...
	 * Parse a custom element (outside of the default namespace).
	 * @param ele the element to parse
	 * @param containingBd the containing bean definition (if any)	为父类bean，对顶层元素的解析应设置为null
	 * @return the resulting bean definition
	 */
	@Nullable
